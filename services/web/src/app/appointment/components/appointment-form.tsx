...
export default function AppointmentForm() {
  ...
  const packages = (): { packages: Package[] } => {
    if (!state.galleryId) return { packages: [] };
    return state.galleries.find(obj => obj.id == state.galleryId) || { packages: [] }
  };

  const formatAddOns = () => {
    const selectedGallery = state.galleries.find(obj => obj.id == state.galleryId);

    if (!selectedGallery) return [];

    return selectedGallery.addOns.map(addOn => ({
      ...
      <Card className="mx-auto shadow-lg border-0 sm:border">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            { state.displayDate && (
              <div className="space-y-2">
                <DatePick onChange={(value) => dispatch({ type: "SET_FIELD", field: "preferredDateTime", value: value })}
                />
              </div>
            )}

            {state.displayLocation && <LocationSearchInput /> }
